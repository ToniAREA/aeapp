...
@can('m_log_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.m-logs.create') }}">
                ...
                <tbody>
                    @foreach($mLogs as $key => $mLog)
                        <tr data-entry-id="{{ $mLog->id }}">
                            ...
                            <td>
                                @can('m_log_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.m-logs.show', $mLog->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('m_log_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.m-logs.edit', $mLog->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('m_log_delete')
                                    <form action="{{ route('admin.m-logs.destroy', $mLog->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('m_log_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.m-logs.massDestroy') }}",
